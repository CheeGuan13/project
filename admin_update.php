<?php
	...
	if(isset($_POST["submit"])) {
		...
		$sql = "UPDATE admins SET admin_id='$admin_id', password='$password', name='$name'
	...
	while($admins = mysqli_fetch_array($result)) {
		$name = $admins["name"];
		$password = $admins["password"]; // Note: this is hashed, so user won't see original password
	}
?>
...
<!-- Form -->
<div class="container">
	<h3 class="text-center mt-5">Update Admin</h3>
	<form action="admin_update.php" method="post" class="mt-3">
		<table class="mx-auto">
			<tr>
				<td>Admin ID</td>
				<td>
					<input type="text" name="admin_id" placeholder="Enter new admin id" value="<?php echo $admin_id; ?>" required>
					<div class="error-msg" id="admin-id-error"></div>
					<input type="hidden" name="original_admin_id" value="<?php echo $admin_id; ?>">
				</td>
			</tr>
			<tr>
				<td>Name</td>
